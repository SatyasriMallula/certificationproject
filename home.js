...
localStorage.setItem("Teams", JSON.stringify(dataofteam));
if(localStorage.getItem("Player") === null)
localStorage.setItem("Player", JSON.stringify(dataofplayers));

let detailofteam = [];
let detailofplayer = [];
detailofteam = JSON.parse(localStorage.getItem("Teams"));
detailofplayer = JSON.parse(localStorage.getItem("Player"));
console.log(dataofteam)
let cardcontainer=document.createElement("div")
cardcontainer.classList.add("cardcontainer")
$("#container1").append(cardcontainer)
let arr=dataofteam.map((ele)=>{
    ...
    teamname.classList.add("teamhead")
    teamname.innerHTML=ele.teamFullName;
    card.append(teamlogo)
    card.append(teamname)
    teamlogo.src=ele.teamIcon; 
    cardcontainer.append(card) 
    card.addEventListener("click",(()=>{
       window.open(`./teamdetails.html?name=${ele.sName}`,'_self')
    }))
})
//search code
var suggestArray=[];
...
function currentLi(element){
    ...
    icon.onclick = ()=>{
        ...
        window.open(`./teamdetails.html?name=${detailofteam.sName}`,"_self")
